...
world.addBody(stick4Body);
  ...
  // ===== 物理：箱（動的）=====
  // worldAABBを直接使って、見た目と物理のズレを抑えつつ
  // 確実に有効な当たり判定を作る
  boxMesh.updateWorldMatrix(true, true);
  const boxWorldAabb = new THREE.Box3().setFromObject(boxMesh);
  const boxSize = new THREE.Vector3();
  const boxCenter = new THREE.Vector3();
  boxWorldAabb.getSize(boxSize);
  boxWorldAabb.getCenter(boxCenter);
  ...
  // 以前の挙動に合わせて少し上から落とす
  boxBody.position.set(boxCenter.x, boxCenter.y + 0.5, boxCenter.z);
