...
const ARM_BODY_SCALE = 0.7; // 本体だけ
const CLAW_SCALE = 1.5; // 爪だけ
const ARM_SCALE = 2; // 全体
const ARM_ROT_SPEED = 0.8;
let CLAW_AXIS = "x";
let CLAW_SIGN = 1;
...
// ステップ管理: 0=待機, 1=開く, 2=下げる, 3=閉じる, 4=上げる(NEW), 5=戻る(NEW), 6=離す(NEW)
let autoStep = 0;
let autoT = 0;
let dropStartY = 0;
...
const matClaw = new CANNON.Material("claw"); // ★新しく爪用の素材を追加
...
// 爪 vs 箱（★ここが重要！滑りにくくする）
world.addContactMaterial(
  new CANNON.ContactMaterial(matClaw, matBox, {
    friction: 0.9,      // ★摩擦係数を大幅アップ（0.05 -> 0.9）
    restitution: 0.0,   // 跳ね返りなし
    contactEquationStiffness: 1e7, // ガッチリ掴むための硬さ設定
...
function animate(t) {
  ...
  const dt = Math.min((t - lastT) / 1000, 1 / 60); // 1/60安定
  lastT = t;
  ...
  if (autoStarted) {
    ...
    // 5. 元の場所に戻る（★追加）
      ...
      const speed = ARM_MOVE_SPEED * dt;
      // Xを戻す
      if (armGroup.position.x > HOME_X) {
        armGroup.position.x = Math.max(HOME_X, armGroup.position.x - speed);
      }
      // Zを戻す
      if (armGroup.position.z < HOME_Z) {
        armGroup.position.z = Math.min(HOME_Z, armGroup.position.z + speed);
      }
      // 到着判定
    ...
    // 6. 開いて景品を落とす（★追加）
      ...
      autoT += dt;
      setClawOpen01(Math.min(autoT / CLAW_OPEN_TIME, 1));
      if (autoT >= CLAW_OPEN_TIME + 0.5) { // 少し待つ
        // リセット処理
        autoStarted = false;
        autoStep = 0;
        phase = 0;
        arrowBtn1.setEnabled(true);
        setClawOpen01(0); // 次のために閉じておく（見た目だけ）
      }
    }
  }
