...
// ===== 逆同期方式（物理Body→メッシュ）=====
const PENETRATION_THRESHOLD = 0.002;          // これ未満の侵入は無視
const PENETRATION_CORRECTION_SCALE = 2.5;     // 侵入量→爪開き角度の補正倍率
const ARM_PENETRATION_PUSHBACK = 0.6;         // 降下中の侵入→アーム押し戻し倍率
const PEN_BLOCK_DECAY_RATE = 0.3;             // 侵入ブロック解除速度（open01/sec）
...
function setClawOpen01(open01, dt = 1 / 60) {
  ...
  // ===== 侵入ブロック：侵入補正で開いた角度より閉じ方向へ進まない =====
  if (penBlockAngleL !== null) {
    // L: CLAW_L_CLOSED(0.4) > CLAW_L_OPEN(-0.3) → 閉=増加方向 → ブロック角度が上限
    if (CLAW_L_CLOSED > CLAW_L_OPEN) nextL = Math.min(nextL, penBlockAngleL);
    else nextL = Math.max(nextL, penBlockAngleL);
  }
  if (penBlockAngleR !== null) {
    // R: CLAW_R_CLOSED(-0.6) < CLAW_R_OPEN(0.2) → 閉=減少方向 → ブロック角度が下限
    if (CLAW_R_CLOSED > CLAW_R_OPEN) nextR = Math.min(nextR, penBlockAngleR);
    else nextR = Math.max(nextR, penBlockAngleR);
  }
...
// ===== 逆同期用：侵入時の最小開度ブロック =====
let penBlockAngleL = null;    // 左爪の侵入ブロック角度（null=ブロックなし）
let penBlockAngleR = null;    // 右爪の侵入ブロック角度（null=ブロックなし）
...
// ===== 逆同期方式：物理ステップ後に Body を正として Mesh を追従させる =====

/**
 * 爪Bodyの接触法線と侵入量から、Bodyの補正ベクトルを計算する。
 * 返り値: { pushVec: CANNON.Vec3, maxPen: number }
 */
function computeClawPushOut(clawBody) {
  if (!clawBody || !boxBody) return { pushVec: new CANNON.Vec3(), maxPen: 0 };
  ...
  for (const c of world.contacts) {
    ...
    if (gap < 0) {
      const pen = Math.abs(gap);
      maxPen = Math.max(maxPen, pen);
      // 法線は bi→bj 方向。clawがbiなら法線を反転（爪を押し出す方向）
      const sign = c.bi === clawBody ? -1 : 1;
      nx += c.ni.x * sign * pen;
      ny += c.ni.y * sign * pen;
      nz += c.ni.z * sign * pen;
      count++;
  ...
  if (count > 0) {
    nx /= count; ny /= count; nz /= count;
  }

  return { pushVec: new CANNON.Vec3(nx, ny, nz), maxPen };
}
 ...
 * 1. 侵入検出 → Body補正位置を算出
 * 2. Body位置を補正
 * 3. Mesh（シーングラフ）をBodyに合わせて逆同期
 * 4. 侵入ブロック角度を設定（autoシーケンスが上書きしないように）
 */
function resolveAndReverseSync(dt) {
  if (!clawLBody || !clawRBody || !armGroup) return;

  const penL = getMaxPenetrationAgainst(clawLBody);
  const penR = getMaxPenetrationAgainst(clawRBody);

  const hasPenL = penL > PENETRATION_THRESHOLD;
  const hasPenR = penR > PENETRATION_THRESHOLD;

  if (!hasPenL && !hasPenR) {
    // ===== 非接触/侵入なし：ブロック角度を徐々に解除 =====
    if (penBlockAngleL !== null) {
      // ブロック角度を閉じ方向へ緩和
      const decayL = PEN_BLOCK_DECAY_RATE * dt * (CLAW_L_OPEN - CLAW_L_CLOSED);
      // 閉じ方向（openDirの逆）に少し戻す
      penBlockAngleL -= decayL;
      // 完全閉じまで戻ったら解除
      const closeEnoughL = CLAW_L_CLOSED > CLAW_L_OPEN
        ? penBlockAngleL >= CLAW_L_CLOSED - 0.01
        : penBlockAngleL <= CLAW_L_CLOSED + 0.01;
      if (closeEnoughL) penBlockAngleL = null;
    }
    if (penBlockAngleR !== null) {
      const decayR = PEN_BLOCK_DECAY_RATE * dt * (CLAW_R_OPEN - CLAW_R_CLOSED);
      penBlockAngleR -= decayR;
      const closeEnoughR = CLAW_R_CLOSED > CLAW_R_OPEN
        ? penBlockAngleR >= CLAW_R_CLOSED - 0.01
        : penBlockAngleR <= CLAW_R_CLOSED + 0.01;
      if (closeEnoughR) penBlockAngleR = null;
    }
    return;
  }
  ...
  // --- 爪の侵入 → ピボット角度を開き方向に補正し、ブロック角度を設定 ---
  if (hasPenL && clawLPivot) {
    ...
    // ブロック角度を設定（次フレームのsetClawOpen01で閉じすぎを防止）
    penBlockAngleL = newAngle;
    corrected = true;
  }

  if (hasPenR && clawRPivot) {
    ...
    penBlockAngleR = newAngle;
    corrected = true;
  }

  // --- 降下中（ステップ2）のアーム押し戻し ---
  if (autoStarted && autoStep === 2) {
  ...
  // --- メッシュ補正後、行列更新 → Body をメッシュに完全一致させる（逆同期完了） ---
  if (corrected) {
    // clawOpen01も補正後の値に合わせる（autoシーケンスの次フレーム開始値として使用）
    clawOpen01 = Math.max(clawOpen01L, clawOpen01R);
    // 行列再計算 → Body再テレポートで Body≡Mesh を保証
    armGroup.updateMatrixWorld(true);
    teleportBodyToMesh(clawLBody, clawLMesh);
    teleportBodyToMesh(clawRBody, clawRMesh);
    if (armBody) {
...
if (autoStarted) {
  ...
  // ===== 物理同期：メッシュ → Body テレポート → Step → 逆同期 =====
  teleportAllBodiesToMesh();

  const MAX_SUB = 8;
  world.step(PHYSICS_FIXED_DT, dt, MAX_SUB);

  // 物理ステップ後：侵入検出 → Body補正 → メッシュ逆同期（Body≡Mesh保証）
  resolveAndReverseSync(dt);

  // ===== 確定状態でビジュアル更新（Body=Meshが保証された状態） =====
  stabilizePrizeBody(boxBody);
  updateClawHitboxVisuals();   // ← 補正後の確定Body状態で爪ワイヤフレーム描画
